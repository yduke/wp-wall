...
Plugin originally by Vladimir Prelovac, updated and maintained by freeduke .
...
= 2.1.0 =
* Security overhaul: migrated AJAX handler from standalone script to WordPress admin-ajax.php API
* Added CSRF/nonce protection for all comment submissions and widget controls
* Fixed SQL injection vulnerability in ShowComments() — switched to wpdb::prepare()
* Added proper input sanitization: sanitize_text_field, sanitize_email, esc_url_raw, wp_kses_post
* Converted widget registration to WP_Widget class for block widget editor compatibility
* Fixed content not updating on pagination clicks; added loading spinner
